...
def preprocess_image(image, model_name, target_size=(224, 224), unet_model=None):
    ...
    # --- Aszimmetrikus crop (tüdőröntgen tipikus elrendezéséhez igazítva) ---
    elif "crop" in name_lower:
        img_uint8 = np.clip(img_array, 0, 255).astype(np.uint8)
        h, w = img_uint8.shape[:2]
        y1 = int(h * 0.15)   # felső 15% levágva (váll, nyak)
        y2 = int(h * 0.90)   # alsó 10% levágva
        x1 = int(w * 0.05)   # bal 5% levágva
        x2 = int(w * 0.95)   # jobb 5% levágva
        img_array = img_uint8[y1:y2, x1:x2]
        img_array = cv2.resize(img_array, target_size)

    # --- Alap resize (crop nélküli modellek) ---
    else:
        img_resized = image.resize(target_size)
        img_array = np.array(img_resized)

    # --- CLAHE (minden modellnél ahol a névben szerepel) ---
    ...
    # --- Modell-specifikus normalizáció ---
                        ...
                        if layer_name:
                            try:
                                ...
                                with col3:
                                    ...
                                    st.info("The heatmap shows where the model focused to make its decision. Red areas indicate the most important regions.")
                            except Exception as e:
